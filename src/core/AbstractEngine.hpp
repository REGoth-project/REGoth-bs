...
namespace REGoth
{
  ...
  /**
   * @brief The core class of REGoth, which offers a lot of default implementations to initializes
   * the engine, set the input, the scene, and others.
   *
   * To handle more use cases, and if a custom `EngineConfig` is desired `AbstractEngine` can be
   * extended (otherwise, see `Engine`).  The base implementations provided here will not load
   * any world and start on an empty scene but with most of the utilities set up to load original
   * content and game mechanics.
   *
   * Therefore, the `AbstractEngine` can be used to implement viewers and other tools, as well as the
   ...
   *
   * - `config()`: Return the engine's configuration. To use a default implementation, see `Engine`.
   *
   * To actually run an instance of the engine, see `runEngine()`.
   *
   * @ingroup core
   * @headerfile core/AbstractEngine.hpp
   */
  ...
  public:
     ...
     * extending `Engine` instead of `AbstractEngine`, which features a default implementation using
     * `EngineConfig`.
     *
